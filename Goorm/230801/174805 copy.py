...
def comp_for_each_elmnt(ex_criteria, each_seq_a):
    ...
    if (len_crit == 1):
        if (len_seq_a == 1):
            return (ex_criteria != each_seq_a)
        else:
            for j in range(len_seq_a):
                if (each_seq_a[j] == ex_criteria):
                    return False
            return True
    ...
    if (len_crit > len_seq_a):
        ...
        for j in range(len_seq_a):
            ...
            if (s == c):
                if (len_seq_a - j >= len_crit):
                    new_seq = each_seq_a[j:(j + len_crit)]
                    if (ex_criteria == new_seq):
                        return False
        return True
...
if __name__ == "__main__":
    while(1):
        ...
        # N
        entire_num = int(input_data[first][0])
